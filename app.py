...
def get_session_info(session_id):
    ...
    cursor = conn.cursor()
    cursor.execute('SELECT country, reputation FROM sessions WHERE id = {placeholder}', (session_id,))
    row = cursor.fetchone()
    conn.close()
...
def check_rate_limit(session_id, action_type):
    ...
    cursor = conn.cursor()
    cursor.execute('SELECT created_at, last_post_at, is_banned FROM sessions WHERE id = {placeholder}', (session_id,))
    row = cursor.fetchone()
    conn.close()
...
def view_board(slug):
    ...
    cursor.execute('SELECT * FROM boards WHERE slug = {placeholder} AND is_active = 1', (slug,))
...
def view_thread(slug, thread_id):
    ...
    cursor.execute('SELECT * FROM boards WHERE slug = {placeholder}', (slug,))
    board = cursor.fetchone()
...
def new_thread(slug):
    ...
    cursor.execute('SELECT id FROM boards WHERE slug = {placeholder} AND is_active = 1', (slug,))
    board = cursor.fetchone()
...
def reply_to_thread(slug, thread_id):
    ...
    cursor.execute('SELECT id FROM boards WHERE slug = {placeholder}', (slug,))
    board = cursor.fetchone()
...
def bury_post(post_id):
    ...
    cursor.execute('SELECT bury_score FROM posts WHERE id = {placeholder}', (post_id,))
...
def nominate_election(slug):
    ...
    cursor.execute('SELECT id FROM boards WHERE slug = {placeholder} AND election_active = 1', (slug,))
    board = cursor.fetchone()
...
def start_board_election(slug):
    ...
    cursor.execute('SELECT id, election_active FROM boards WHERE slug = {placeholder}', (slug,))
    board = cursor.fetchone()
