...
def get_session_info(session_id):
    ...
    cursor = conn.cursor()
    cursor.execute('SELECT country, reputation FROM sessions WHERE id = ?', (session_id,))
    row = cursor.fetchone()
    conn.close()
...
def start_election(board_id):
    ...
    try:
        ...
        # Update board
        cursor.execute('''
            UPDATE boards SET election_active = 1, election_ends_at = ?
            WHERE id = ?
        ''', (ends_at, board_id))
...
def nominate_for_election(election_id, session_id, statement):
    ...
    try:
        ...
        cursor.execute('''
            ...
            WHERE election_id = ? AND session_id = ?
...
def end_election(election_id):
    ...
    try:
        ...
        if winner and winner['votes'] > 0:
            # Record winner
            cursor.execute('''
                UPDATE elections SET winner_session_id = ?, is_active = 0
                WHERE id = ?
...
def calculate_trending():
    ...
    for board in boards:
        ...
        cursor.execute('''
            UPDATE boards 
            SET activity_score = ?, post_count = ?, unique_posters = ?
            WHERE id = ?
...
def get_global_stats():
    ...
    cursor.execute('SELECT COUNT(*) as count FROM boards WHERE is_active = 1')
...
def check_rate_limit(session_id, action_type):
    ...
    cursor = conn.cursor()
    cursor.execute('SELECT created_at, last_post_at, is_banned FROM sessions WHERE id = ?', (session_id,))
    row = cursor.fetchone()
    conn.close()
...
def update_post_timestamp(session_id):
    ...
    if conn:
        ...
        cursor.execute('''
            UPDATE sessions 
            SET last_post_at = ?, post_count = post_count + 1 
            WHERE id = ?
...
def view_board(slug):
    ...
    cursor.execute('SELECT * FROM boards WHERE slug = ? AND is_active = 1', (slug,))
    ...
    # Check if mod
    cursor.execute('''
        SELECT role FROM moderators WHERE board_id = ? AND session_id = ?
    ''', (board['id'], session_id))
    ...
    if board['election_active']:
        cursor.execute('''
            ...
            FROM elections e
            WHERE e.board_id = ? AND e.is_active = 1
            ORDER BY e.id DESC
            LIMIT 1
        ''', (board['id'],))
    ...
    cursor.execute('''
        ...
        WHERE p.board_id = ? AND p.parent_id IS NULL AND p.is_deleted = 0
        GROUP BY p.id
...
def view_thread(slug, thread_id):
    ...
    cursor.execute('''
        SELECT * FROM posts 
        WHERE id = ? AND board_id = ? AND parent_id IS NULL AND is_deleted = 0
    ''', (thread_id, board['id']))
...
def new_thread(slug):
    ...
    cursor.execute('SELECT id FROM boards WHERE slug = ? AND is_active = 1', (slug,))
    board = cursor.fetchone()
...
def reply_to_thread(slug, thread_id):
    ...
    cursor.execute('SELECT id FROM boards WHERE slug = ?', (slug,))
    board = cursor.fetchone()
...
def bury_post(post_id):
    ...
    cursor.execute('''
        UPDATE posts SET bury_score = bury_score + 1 WHERE id = ?
    ''', (post_id,))
    
    cursor.execute('SELECT bury_score FROM posts WHERE id = ?', (post_id,))
...
def mod_delete(slug, post_id):
    ...
    cursor.execute('''
        UPDATE posts SET is_deleted = 1 WHERE id = ? AND board_id = ?
    ''', (post_id, board['id']))
...
def view_election(slug):
    ...
    for candidate in candidates:
        ...
        candidates_with_info.append(cand_dict)
...
def nominate_election(slug):
    ...
    cursor.execute('SELECT id FROM boards WHERE slug = ? AND election_active = 1', (slug,))
    board = cursor.fetchone()
...
def start_board_election(slug):
    ...
    cursor.execute('SELECT id, election_active FROM boards WHERE slug = ?', (slug,))
    board = cursor.fetchone()
...
if __name__ == '__main__':
    debug_print("Starting 16chan V3...")
    init_db()
